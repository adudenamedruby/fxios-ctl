# Guidelines on Adding Functionality to `fxios`

## What should be added to `fxios`

### 1. Workflows that map to CI / GitHub Actions

If the repo runs a task in CI (build, lint, test, package, verify), it’s a strong candidate to be runnable via `fxios`.

> Rule: If CI runs it, `fxios` should be able to run it locally with the same defaults (or explain what’s different).
...
…then the `fxios` should probably encode the correct path.
...
If it depends on repo conventions (schemes, paths, configs), it belongs in `fxios` instead of a generic script.
...
## What should NOT be added to `fxios`
...
If `fxios` foo is just somecommand foo with no defaults, no validation, no repo-specific behavior, don’t add it.

> Rule: `fxios` should not be a thin alias collection.
...
`fxios` should work in CI and scripts. Interactive prompts must be avoidable.
...
- `fxios` test unit|ui|smoke
- `fxios` lint [--changed|--all|--ci]
- `fxios` clean deriveddata|spm|artifacts|sim
- `fxios` sim list|boot|shutdown|erase
- `fxios` version show|bump|set|verify
...
- they’re extremely common (`fxios` b → `fxios` build)
...
- clearly separate `fxios` output from passthrough tool output
...
3. Once it has stable semantics, wrap it as `fxios` <domain> <subcommand>

If it becomes part of CI, it must be runnable via `fxios ci …`

> Rule: Don’t add “brand new” workflows directly to `fxios` unless they’re immediately required for CI parity.

## Design principle: `fxios` is opinionated, not everything

When a tradeoff exists, `fxios` should encode the repo’s preferred defaults, but allow escape hatches.
