...
/// Configuration loaded from .fxios.yaml at the repository root.
struct FxiosConfig: Codable {
    let project: String
...
/// Project config (.fxios.yaml) takes precedence over DefaultConfig values.
struct MergedConfig {
    let project: String
    let defaultBootstrap: String
    let defaultBuildProduct: String

    init(projectConfig: FxiosConfig) {
...
enum RepoDetectorError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .markerNotFound:
            return """
                Not a fxios-compatible repository.
...
/// 2. Repository root contains .fxios.yaml marker file
...
enum RepoDetector {
    ...
    static func loadConfig(from markerPath: URL) throws -> FxiosConfig {
        ...
        do {
            let config = try YAMLDecoder().decode(FxiosConfig.self, from: contents)
    ...
    /// Uses git to find the repo root, then checks for .fxios.yaml marker.
