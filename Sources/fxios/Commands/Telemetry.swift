...
struct Telemetry: ParsableCommand {
    ...
    private func runRefresh(repoRoot: URL) throws {
        ...
        Herald.declare("Successfully updated the xcode build phase infile list.")
    }
    ...
    private func runAdd(featureName: String, description: String?, repoRoot: URL) throws {
        ...
        if try updateTagsYaml(at: tagsFile, tagName: capitalizedTag, description: description) {
            Herald.declare("Successfully added \(capitalizedTag) tag to tags.yaml")
            if description == nil {
                Herald.declare("Please update the description in tags.yaml for the \(capitalizedTag) tag")
            }
        }
    }
    ...
    private func updateXcodeFileList(repoRoot: URL) throws {
        ...
        var content = "# This is an autogenerated file using fxios telemetry --refresh\n"
    ...
    private func updateTagsYaml(at url: URL, tagName: String, description: String?) throws -> Bool {
        ...
        if content.contains("\(tagName):") {
            Herald.declare("Tag \(tagName) already exists in tags.yaml")
            return false
        }
